...
class TestComposedListFilters(ComposedTestSetup):
    def test_list_filter_by_name(self, repository, category):
        create_result_1 = repository.create(ComposedCreateRequest(name="Target Item", category_id=category.id))

        assert isinstance(create_result_1, SuccessDTO)

        create_result_2 = repository.create(ComposedCreateRequest(name="Other Item", category_id=category.id))
    ...
    def test_list_filter_by_status(self, repository, category):
        repository.create(ComposedCreateRequest(name="Active Item", category_id=category.id, status="active"))
        repository.create(ComposedCreateRequest(name="Inactive Item", category_id=category.id, status="inactive"))
...
class TestComposedListPagination(ComposedTestSetup):
    def test_pagination_limit(self, repository, category):
        for i in range(5):
            repository.create(ComposedCreateRequest(name=f"Item {i}", category_id=category.id))
    ...
    def test_pagination_offset(self, repository, category):
        items = []
        for i in range(3):
            created = repository.create(ComposedCreateRequest(name=f"Item {i}", category_id=category.id))
    ...
    def test_filters(self, repository, category):
        for i in range(4):
            repository.create(ComposedCreateRequest(name=f"Test {i}", category_id=category.id, status="active"))
...
class TestComposedListPaginationValidation(ComposedTestSetup):
    ...
    def test_has_next_page_no_pagination(self, repository, category):
        repository.create(ComposedCreateRequest(name=f"Test Item", category_id=category.id))
