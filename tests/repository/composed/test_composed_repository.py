...
from tests.repository.composed.composed_secondary_entities import (
    ...
    ComposedListRequest,
    ComposedTestSetup, ComposedRepository, ComposedListDTO, ComposedCreateDTO,
)
...
class TestComposedListFilters(ComposedTestSetup):
    def test_list_filter_by_name(
        ...
        result: ComposedListDTO = repository.list(
            ComposedListRequest(name="Target Item")
        )
    ...
    def test_list_filter_by_status(
        ...
        result: ComposedListDTO = repository.list(
            ComposedListRequest(status="inactive")
        )
...
class TestComposedListPagination(ComposedTestSetup):
    def test_pagination_limit(
        ...
        list_result: ComposedListDTO = repository.list(
            ComposedListRequest(page=1, page_size=2)
        )
    ...
    def test_pagination_offset(
        ...
        result: ComposedListDTO = repository.list(
            ComposedListRequest(page=2, page_size=1)
        )
...
class TestComposedListPaginationValidation(ComposedTestSetup):
    def test_has_next_page_single_page(
            self,
            repository: ComposedRepository,
            category: CategoryCoreModel
    ) -> None:
        result: ComposedListDTO = repository.list(
            ComposedListRequest(page=1, page_size=10)
        )
    ...
    def test_has_next_page_multiple_pages(
            self,
            repository: ComposedRepository,
            category: CategoryCoreModel
    ) -> None:
    ...
    def test_has_next_page_last_page(
            self,
            repository: ComposedRepository,
            category: CategoryCoreModel
    ) -> None:
        ...
        list_result: ComposedListDTO = repository.list(
            ComposedListRequest(page=2, page_size=10)
        )
    ...
    def test_has_next_page_empty_result(
            self,
            repository: ComposedRepository
    ) -> None:
        result: ComposedListDTO = repository.list(
    ...
    def test_has_next_page_no_pagination(
        ...
        result: ComposedCreateDTO = repository.create(
            ComposedCreateRequest(name="Test Item", category_id=category.id)
        )
        if isinstance(result, BaseError):
